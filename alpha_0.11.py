...
class Turniermanager(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.create_bounty_mode_selector()  # Neues GUI-Element zum Umschalten der Bounty-Modi


    def create_bounty_mode_selector(self):
        # Frame für die Bounty-Modus Auswahl
        bounty_mode_frame = ttk.Frame(self.tournament_frame, borderwidth=2, relief="solid")
        bounty_mode_frame.grid(row=0, column=2, padx=10, pady=10, sticky="w")

        # Label für den Bounty-Modus
        ttk.Label(bounty_mode_frame, text="Bounty Mode:", font=("Helvetica", 16)).grid(row=0, column=0, padx=10, pady=5)

        # Combobox für die Auswahl des Bounty-Modus
        self.bounty_mode_var = tk.StringVar()
        bounty_modes = ["Fixed", "Progressive"]
        self.bounty_mode_combobox = ttk.Combobox(bounty_mode_frame, textvariable=self.bounty_mode_var, values=bounty_modes, state="readonly", font=("Helvetica", 16))
        self.bounty_mode_combobox.grid(row=0, column=1, padx=10, pady=5)
        self.bounty_mode_combobox.current(0)  # Setze den Standardwert auf "Fixed"
        self.bounty_mode_combobox.bind("<<ComboboxSelected>>", self.on_bounty_mode_change)

    def on_bounty_mode_change(self, event):
        selected_mode = self.bounty_mode_var.get()
        if selected_mode == "Fixed":
            self.bounty_manager.set_mode("fixed")
        elif selected_mode == "Progressive":
            self.bounty_manager.set_mode("progressive")
        self.update_bounty_earnings()

    def update_bounty_earnings(self):
        """Aktualisiert die Bounty Earnings für alle Spieler basierend auf dem aktuellen Modus."""
        for item in self.player_tree.get_children():
            values = self.player_tree.item(item, "values")
            player_name = values[0]
            knockouts = int(values[7])  # Stelle sicher, dass knockouts ein int ist
            bounty_price = self.bounty_manager.bounty_price
            bounty_earnings = knockouts * bounty_price  # Berechnung Bounty Earnings
...
class BountyManager:
    def __init__(self, initial_bounty=0):
        self.bounty_price = 0
        self.knockouts = {}
        self.bounty_pool = {}
    ...
    def record_knockout(self, eliminator, eliminated):
        if eliminator not in self.knockouts:
            self.knockouts[eliminator] = 0
        if eliminated not in self.bounty_pool:
            self.bounty_pool[eliminated] = self.initial_bounty
        
        self.knockouts[eliminator] += 1
        
        if self.mode == "progressive":
            earned_bounty = self.bounty_pool[eliminated] / 2
            self.bounty_pool[eliminator] += earned_bounty
            self.payout_bounty(eliminator, earned_bounty)
        else:
            self.payout_bounty(eliminator, self.bounty_price)

    def payout_bounty(self, player, amount):
        # Logik zur Auszahlung des Bounty-Betrags an den Spieler
        print(f"{player} erhält {amount:.2f} als Bounty.")

    def calculate_bounty_earnings(self, player):
        if self.mode == "progressive":
            return self.get_bounty_pool(player)
        else:
            return self.knockouts.get(player, 0) * self.bounty_price

    def get_total_knockouts(self, player):
        return self.knockouts.get(player, 0)

    def get_bounty_pool(self, player):
        return self.bounty_pool.get(player, self.initial_bounty)
